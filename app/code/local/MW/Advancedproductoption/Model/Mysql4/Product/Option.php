 ...
 */
class MW_Advancedproductoption_Model_Mysql4_Product_Option extends Mage_Core_Model_Mysql4_Abstract
{
    protected function  _construct()
    {
        $this->_init('advancedproductoption/product_option', 'option_id');
    }

    protected function _afterSave(Mage_Core_Model_Abstract $object)
    {
        $priceTable = $this->getTable('advancedproductoption/product_option_price');
        $titleTable = $this->getTable('advancedproductoption/product_option_title');
    ...
    public function deletePrices($option_id)
    {
        ...
        $this->_getWriteAdapter()->delete(
            $this->getTable('advancedproductoption/product_option_price'),
            $condition);
    ...
    public function deleteTitles($option_id)
    {
        ...
        $this->_getWriteAdapter()->delete(
            $this->getTable('advancedproductoption/product_option_title'),
            $condition);
    ...
    public function duplicate(Mage_Catalog_Model_Product_Option $object, $oldProductId, $newProductId)
    {
        ...
        $select = $read->select()
            ->from($this->getTable('advancedproductoption/product_option'))
        ...
        foreach ($optionsCond as $oldOptionId => $newOptionId) {
            // title
            $table = $this->getTable('advancedproductoption/product_option_title');
            ...
            // price
            $table = $this->getTable('advancedproductoption/product_option_price');
            $sql = 'REPLACE INTO `' . $table . '` '
    ...
    public function getSearchableData($productId, $storeId)
    {
        ...
        $select = $this->_getReadAdapter()->select()
            ...
            ->join(
                array('option_title_default' => $this->getTable('advancedproductoption/product_option_title')),
                'option_title_default.option_id=option.option_id AND option_title_default.store_id=0',
                array())
            ->joinLeft(
                array('option_title_store' => $this->getTable('advancedproductoption/product_option_title')),
        ...
        $select = $this->_getReadAdapter()->select()
            ->from(array('option' => $this->getMainTable()), null)
            ->join(
                array('option_type' => $this->getTable('advancedproductoption/product_option_type_value')),
                'option_type.option_id=option.option_id',
                array())
            ->join(
                array('option_title_default' => $this->getTable('advancedproductoption/product_option_type_title')),
                'option_title_default.option_type_id=option_type.option_type_id AND option_title_default.store_id=0',
                array())
            ->joinLeft(
                array('option_title_store' => $this->getTable('advancedproductoption/product_option_type_title')),
