 ...
 */
class MW_Advancedproductoption_Model_Mage_Product_Option_Type_Default extends Varien_Object
{
    /**
     * Option Instance
     *
    ...
    {
    	//echo(get_class(Mage::getModel('catalog/product_option_type_default'))); die();
        ...
        $mw_condition = !isset($values[$option->getId()]) && $option->getIsRequire() && !$this->getSkipCheckRequiredOption();
    	if($enabled_module){
			...
			if($check_option_qty) $mw_condition = !isset($values[$option->getId()]) && $option_require && !$this->getSkipCheckRequiredOption();
	        ...
	        //if (!isset($values[$option->getId()]) && $option_require && !$this->getSkipCheckRequiredOption()) {
    	}
        if ($mw_condition) {
    ...
    public function getOptionPrice($optionValue, $basePrice)
    {
	    ...
	    if ($option->getPriceType() == 'abs') {
	        return array('abs', $option->getPrice());
	    }
	    else {
	        return $this->_getChargableOptionPrice(
	            $option->getPrice(),
