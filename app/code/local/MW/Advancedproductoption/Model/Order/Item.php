 ...
 */
class MW_Advancedproductoption_Model_Order_Item extends Mage_Sales_Model_Order_Item
{
    ...
    public function getProductOptions()
    {
        if ($options = $this->_getData('product_options')) {
        	...
        	if (isset($options['options'])) {
        		//$options_news = array_merge($options_news, $options['options']);
        		foreach ($options['options'] as $mw_option) {
        			$option_id = $mw_option['option_id'];
        			$options_news['label'] = $mw_option['label'];
        			$options_news['value'] = $this->getMwFormattedOptionValue($mw_option,$mw_option['option_value'],$options['info_buyRequest'],$mw_option['value']);
        			$options_news['print_value'] = $this->getMwFormattedOptionValue($mw_option,$mw_option['option_value'],$options['info_buyRequest'],$mw_option['value']);
        			/*if(isset($options['info_buyRequest'][$mw_name]) && $options['info_buyRequest'][$mw_name] != '')
        			{
        				$options_news['value'] = $options['info_buyRequest'][$mw_name].' x '. $mw_option['value'];
        				$options_news['print_value'] = $options['info_buyRequest'][$mw_name].' x '.$mw_option['print_value'];
        				//echo $mw_option['value'];die();
        			}else{
        				$options_news['value'] = $mw_option['value'];
        				$options_news['print_value'] = $mw_option['print_value'];
        			}*/
        			$options_news['option_id'] = $mw_option['option_id'];
        			$options_news['option_type'] = $mw_option['option_type'];
        			$options_news['option_value'] = $mw_option['option_value'];
        			$options_news['custom_view'] = $mw_option['custom_view'];
        			$_options_news[]= $options_news;
        		}
        	}
        	if (isset($_options_news)) {
                $result['options'] = $_options_news;
            }
        	if (isset($options['info_buyRequest'])) {
                $result['info_buyRequest'] = $options['info_buyRequest'];
            }
            if (isset($options['additional_options'])) {
                $result['additional_options'] = $options['additional_options'];
            }
            if (isset($options['attributes_info'])) {
                $result['attributes_info'] = $options['attributes_info'];
            }
            return $result;
        }
        return array();
    }
	public function getMwFormattedOptionValue($mw_option,$optionValue,$mw_data,$result_old)
	{
		 $result = '';
		 ...
		 if(sizeof(explode(',', $optionValue)) >1){
		 	$array_title = array();
		 	foreach (explode(',', $result_old) as $key_title=> $_value_title) {
		 		$array_title[$key_title] = $_value_title;
		 	}
			  foreach (explode(',', $optionValue) as $key=> $_value) {
                ...
                if(isset($mw_data[$mw_name]) && $mw_data[$mw_name] != ''){
			 	...
			 	if ($array_title[$key] != '') {
	                    $result .= $mw_qty_show.$array_title[$key] . ', ';
                }else{
                	$result = '';
                }
			 }
			 if($check == 0) return $result_old;
