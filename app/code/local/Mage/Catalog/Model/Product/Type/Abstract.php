...
abstract class Mage_Catalog_Model_Product_Type_Abstract
{
    ...
    {
    	//zend_debug::dump($product->getCustomOption('option_ids'));die();
    	$qty = 1;
    	$product_id = $product ->getId();
		$items = Mage::getSingleton('checkout/session')->getQuote()->getAllItems();
 
		foreach($items as $item) {
			if((int)$item->getProductId() == (int)$product_id) $qty = (int)$item->getQty();
		}
    	
		//zend_debug::dump($cart);die();
		//if($product['has_options'] && $product->getCustomOption('option_ids')){
		if($product['has_options'] && $product->getCustomOption('option_ids')){
    	if ($optionIds = $product->getCustomOption('option_ids')->getValue()) {
        	$mw_data = unserialize($product->getCustomOption('info_buyRequest')->getValue());
            foreach (explode(',', $optionIds) as $optionId) {
                if ($option = $product->getOptionById($optionId)) {
                    ...
                    {
                    	$check_qty_cart = Mage::helper('advancedproductoption')->checkOptionQtyCart($option,$confItemOption->getValue(),$mw_data,$qty);
                    	if($check_qty_cart == 0){
                    		$this->getProduct($product)->setIsValid(false);
        ...
        if (!$this->getProduct($product)->getSkipCheckRequiredOption()) {
            foreach ($this->getProduct($product)->getOptions() as $option) {
            	...
            	$mw_condition = $option->getIsRequire();
    			if($enabled_module){
					...
					if($check_option_qty) $mw_condition = $option_require;
