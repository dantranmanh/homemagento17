...
class Mage_Catalog_Model_Product_Type_Price
{
    ...
    protected function _applyOptionsPrice($product, $qty, $finalPrice)
    {
        if ($optionIds = $product->getCustomOption('option_ids')) {
        	$data = unserialize($product->getCustomOption('info_buyRequest')->getValue());
            ...
            foreach (explode(',', $optionIds->getValue()) as $optionId) {
                if ($option = $product->getOptionById($optionId)) {
                    ...
                    $mw_type = $option->getGroupByType($option->getType());
                    if( $mw_type == 'text'|| $mw_type == 'file' ||  $mw_type == 'date' ){
                    	if($option->getPriceType() == 'abs' &&  $option->getPrice() > 0) {
		                	$value_absolute = 'mw_other'.$option->getOptionId();
		                    $finalPrice = $group->getOptionPrice($confItemOption->getValue(), $basePrice);
	                    }
                    	
                    }else if($mw_type == 'select'){
		                foreach(explode(',', $confItemOption->getValue()) as $value) {
			                $_result = $option->getValueById($value);
			                if($_result->getPriceType() == 'abs' &&  $_result->getPrice() > 0) {
			                	$value_absolute = $value;
			                    $finalPrice = $group->getOptionPrice($value, $basePrice,$data,$optionId);;
		                    }
		                }
                    }
        	...
        	foreach (explode(',', $optionIds->getValue()) as $optionId) {
                if ($option = $product->getOptionById($optionId)) {
                    ...
                    $group = $option->groupFactory($option->getType())
                        ->setOption($option)
                        ->setConfigurationItemOption($confItemOption);
                    //$finalPrice += $group->getOptionPrice($confItemOption->getValue(), $basePrice,$data,$optionId);
                    $mw_type = $option->getGroupByType($option->getType());
                    if( $mw_type == 'text'|| $mw_type == 'file' ||  $mw_type == 'date' ){
                    	if($option->getPriceType() == 'abs' &&  $option->getPrice() > 0) {
                    		$value_other = 'mw_other'.$option->getOptionId();
		                    if($value_other == $value_absolute ){
		                    	$finalPrice += 0;
		                    }
		                    else {
		                    	$finalPrice += $group->getOptionPrice($confItemOption->getValue(), $basePrice);
	                    	...
	                    	if($option->getPriceType()=='onetime') $finalPrice += $group->getOptionPrice($confItemOption->getValue(), $basePrice)/$qty; 
	                    	else $finalPrice += $group->getOptionPrice($confItemOption->getValue(), $basePrice); 
	                    }
		                ...
		                foreach(explode(',', $confItemOption->getValue()) as $value) {
			                $_result = $option->getValueById($value);
			                if($_result->getPriceType() == 'abs' && $_result->getPrice() > 0 ) {
			                    if($value == $value_absolute ){
			                    	$finalPrice += 0;
			                    }
			                    else {
			                    	$finalPrice += $group->getOptionPrice($value, $basePrice,$data,$optionId); 
			                    	//echo  'bbbb'.$group->getOptionPrice($confItemOption->getValue(), $basePrice,$data,$optionId);
			                    }
		                    }else{
		                    	if($_result->getPriceType()=='onetime') $finalPrice += $group->getOptionPrice($value, $basePrice,$data,$optionId)/$qty; 
		                    	else  $finalPrice += $group->getOptionPrice($value, $basePrice,$data,$optionId);
		                    	//echo  'aaa'.$group->getOptionPrice($confItemOption->getValue(), $basePrice,$data,$optionId);
		                    }
		                }
                    }
